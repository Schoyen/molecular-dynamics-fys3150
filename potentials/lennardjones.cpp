...
void LennardJones::calculateForces(System *system)
{
    ...
    //std::cout << celllist->listOfCells()[0]->position << "\n" << std::endl;
    /*
    std::cout << celllist->listOfCells()[63]->position << std::endl;
    temp = celllist->listOfCells()[0]->position - celllist->listOfCells()[63]->position;
    x = temp.x();
    y = temp.y();
    z = temp.z();
    ...
    std::cout << 3 * pow(celllist->getrcut(), 2) << std::endl;
    */
    for (int i = 0; i < (int) celllist->listOfCells().size(); i++) {
        for (int j = 0; j < (int) celllist->listOfCells().size(); j++) {
            if (i != j) {
                ...
                temp = vec3(x, y, z);
                //std::cout << temp << std::endl;
                // Some cells are not computed.
                if (temp.lengthSquared() <= 3 * pow(celllist->getrcut(), 2)) {
                    /*
                    if (i == 0) {
                        std::cout << celllist->listOfCells()[j]->position << std::endl;
                    }
                    */
                    for (int k = 0; k < (int) celllist->listOfCells()[i]->atomsClose().size(); k++) {
                        // Calculation of force inside each cell.
                        // Avoid calculating the force in each cell locally several times.
                        // Make sure all cells are computed.
